...
#        - broken triggers from migration:
#        - !followers !friends !rate
#        - !rtbyme !rttome !public
# v6.01dc-corrected ID placements which fixes issues with
...
# v6.01db-stuff.. did some stuff
# v6.01da-corrected display elements regarding
#        retweets and other details. work in
#        progess. may still need more work.
#        crosses fingers...
# v6.01d- major addition: added retweets:
...
# (string)
set twitter(tweet) "https://api.twitter.com/1.1/statuses/update.json"
set twitter(private_new) "https://api.twitter.com/1.1/direct_messages/new.json"
set twitter(follow) "https://api.twitter.com/1.1/friendships/create.json"
set twitter(unfollow) "https://api.twitter.com/1.1/friendships/destroy.json"
...
set twitter(followers_new) "https://api.twitter.com/1.1/followers/ids.json"
set twitter(users_show) "https://api.twitter.com/1.1/users/lookup.json"
#set twitter(following) "https://api.twitter.com/1.1/statuses/friends.json"
set twitter(following) "https://api.twitter.com/1.1/friends/list.json"
set twitter(following_new) "https://api.twitter.com/1.1/friends/ids.json"
set twitter(private_destroy) "https://api.twitter.com/1.1/direct_messages/destroy.json"
set twitter(tweet_destroy) "https://api.twitter.com/1.1/statuses/destroy/%id%.json"
set twitter(retweet) "https://api.twitter.com/1.1/statuses/retweet/%id%.json"
set twitter(retweets_of_me) "https://api.twitter.com/1.1/statuses/retweets_of_me.json"
set twitter(retweets_to_me) "https://api.twitter.com/1.1/statuses/retweeted_to_me.json"
set twitter(retweets_by_me) "https://api.twitter.com/1.1/statuses/retweeted_by_me.json"
...
# (strings)
# deprecated -- set twitter(friends) "http://api.twitter.com/1.1/statuses/friends_timeline.json"
set twitter(friends) "https://api.twitter.com/1.1/statuses/home_timeline.json"
set twitter(public) "https://api.twitter.com/1.1/statuses/public_timeline.json"
set twitter(user) "https://api.twitter.com/1.1/statuses/user_timeline.json"
set twitter(userxml) "https://api.twitter.com/1.1/statuses/user_timeline.xml"
set twitter(home) "https://api.twitter.com/1.1/statuses/home_timeline.json"
set twitter(mention) "https://api.twitter.com/1.1/statuses/mentions_timeline.json"
set twitter(mentionxml) "https://api.twitter.com/1.1/statuses/mentions.xml"
set twitter(private) "https://api.twitter.com/1.1/direct_messages.json"
set twitter(private_sent) "https://api.twitter.com/1.1/direct_messages/sent.json"
set twitter(rate) "https://api.twitter.com/1.1/get/bittersweet/rate_limit_status.json"
set twitter(search) "https://api.twitter.com/1.1/search/tweets.json"
...
# (integer)
set twitter(fixMyDuration) "3740"
...
# (list)
set twitter(accts) {
	"#roms-isos|suqmuhnutz|hTnRKtbTLvVWAyfVvHg|ZrtGLRJVOl0tYAkicsHJVOBv0aBaNgKEsGbGWQfdWg|109926964-2x7QmerHbrj0ImSy0T6zwu1YiQkASI5rZtzup2WS|Fqms1BxZ5y98JoipbX3TBJKWdqUJ6Bw4zvCEcE8ThM"
	"#eggtcl|suqmuhnutz|hTnRKtbTLvVWAyfVvHg|ZrtGLRJVOl0tYAkicsHJVOBv0aBaNgKEsGbGWQfdWg|109926964-2x7QmerHbrj0ImSy0T6zwu1YiQkASI5rZtzup2WS|Fqms1BxZ5y98JoipbX3TBJKWdqUJ6Bw4zvCEcE8ThM"
	"#maryjane|suqmuhnutz|hTnRKtbTLvVWAyfVvHg|ZrtGLRJVOl0tYAkicsHJVOBv0aBaNgKEsGbGWQfdWg|109926964-2x7QmerHbrj0ImSy0T6zwu1YiQkASI5rZtzup2WS|Fqms1BxZ5y98JoipbX3TBJKWdqUJ6Bw4zvCEcE8ThM"
	"#eggdrop|suqmuhnutz|hTnRKtbTLvVWAyfVvHg|ZrtGLRJVOl0tYAkicsHJVOBv0aBaNgKEsGbGWQfdWg|109926964-2x7QmerHbrj0ImSy0T6zwu1YiQkASI5rZtzup2WS|Fqms1BxZ5y98JoipbX3TBJKWdqUJ6Bw4zvCEcE8ThM"
}
...
if {[catch {package require OAuthSingle 1.0} e] !=0} {
	putlog "Twitter.tcl requires the OAuthSingle package 1.1 (oauth.tcl v1.1) loaded FIRST. Not loaded.."
	return
}
...
proc proc:twitter:duration {date {value 0}} {
	...
	set ago [expr {[clock seconds] - [clock scan "$newdate" -gmt 1] - $::twitter(fixMyDuration)}]] ; if {$ago < 0} { set negative 1 }
	set ago [duration [string map [list "-" ""] $ago]]
	set ago [string map {" years " "y, " " year " "y, " " weeks" "w, " " week" "w, " " days" "d, " " day" "d, " " hours" "h, " " hour" "h, " " minutes" "m, " " minute" "m, " " seconds" "s, " " second" "s,"} $ago]
	if {![info exists negative]} { return "[string trim $ago ", "] ago" } { return "[string trim $ago ", "] from now" }
}
...
proc proc:twitter:restlist {type nick uhost handle chan text {auto 0}} {
	...
	if {[string match "*\"error*" $html]} {
		...
		set thelist [regexp -all -inline {"screen_name"\:"(.*?)"} $html]
		set count 0 ; set out ""
		foreach {j s} $thelist { lappend out $s } ; set out [lsort -unique $out]
		while {[llength $out]} {
			putserv "$to :$twitter(logo) \[$chan@$tuser\] ($type): [join [lrange $out 0 19] ", "]"
			set out [lrange $out 20 end]
		}
	}
}
...
proc proc:twitter:megahal {m h d mo y {id ""} {text ""}} {
	global twitter
	set s "" ; putlog "inside megahal"
	if {[info exists twitter(base64)]} { return }
	foreach entry $twitter(accts) {
		set part [split $entry |]
		set chan [lindex $part 0]
...
proc proc:twitter:restapi {nick uhost hand chan text type} {
	...
	if {$found != -1} {
      ...
      if {[string match "retweet*" $type]} {
		...
		if {![isbotnick $nick]} { lappend q "[list count 100]" }
		set type "search"
	}
...
proc proc:twitter:jsondecode:restapi {html} {
	...
	foreach entry [split $html "\n"] {
            ...
            #} else { set entities "" }
            #if {[regexp -nocase {"user_mentions"\:(.*?)$} $entry - user_mentions]} {
            #   regsub -nocase {"user_mentions"\:.*?$} $entry "" entry
            #} else { set user_mentions "" }
		...
		#		set ust [string map { \] "" \} "" } $ust]
		#		set uen [string map { \] "" \} "" } $uen]
		...
		if {[info exists n] && [string length $id]} { lappend friendslist [list "[proc:twitter:duration $c]" "$id" "[string map [list \\ \\\\ ] $t]" "$so" "$id" "$n" "$s" "$rid" "$rn"] ; unset n ; unset id }
	}
	return $friendslist
}
...
proc proc:twitter:jsondecode:search {html} {
	...
	foreach entry [split $html "\n"] {
		...
		regexp -nocase {"user"\:.*?"screen_name"\:"(.*?)"} $entry - s
		set n ""
		regsub -all -- {<.*?>} [proc:twitter:descdecode [proc:twitter:descdecode $so]] "" so
            #if {[regexp -nocase {"urls"\:(.*?)\]\,} $entities - urlscrub]} {
            #   set urlscrub [string range $urlscrub 2 end]
...
proc proc:tweet {nick uhand handle chan input} {
	...
	if {[string match "*\"error*" $html]} {
		...
		putserv "$to :$twitter(logo) $tt"
		proc:twitter:megahal 0 0 0 0 0 $id $tweetform
...
putlog "Twitter, Tweets with Megahal v6.01da (Idea and original script by Warlord, Super-Action rocket missles by speechles) Loaded.."
