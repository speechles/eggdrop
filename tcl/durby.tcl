...
variable durbyShowMisdetection 0
# ignore urls matching ignore pattern?
# enabling this option will set durby to ignore urls that match
...
# any of these patterns will be ignored if durbyPatternIgnore option is
# enabled
variable patterns {
  *.jpg
  *.png
...
# +-+
#
# Multiple urls in one line of text? 0 = first is all shown, 1 = more than 1 can be used per line
# --- [ 0 off / 1 on ]
# +-+
...
if {![catch {package require tls}]} { set durbyg 1; ::http::register https 443 ::tls::socket }
...
setudef flag durby
bind pub - !durby durby
bind pubm - * weburlwatch
bind pub - !durbycrumble durbycrumble
bind pub - !durbycookie durbycookie
...
proc weburlwatch {nick host user chan text} {
	...
	foreach word "[split $text]" {
		if {[string match -nocase "!webby*" $word] || [string match -nocase "!durby*" $word]} {
			...
			set junk [webby $::botnick $::botnick $::botnick $chan $word]
		}
      }
...
proc durbycrumble {nick uhost handle chan text} {
  if {![channel get $chan durby]} { return }
  global durbyCookie
  if {![info exists durbyCookie]} { putserv "privmsg $chan :\002webby\002: Cookie does not exist." ; return }
  putserv "privmsg $chan :\002webby\002: Crumbling \"$::durbyCookie\"..."
  set durbyCookie ""
}

proc durbycookie {nick uhost handle chan text} {
  if {![channel get $chan durby]} { return }
  global durbyCookie
  if {[info exists durbyCookie]} {
    if {[string length $durbyCookie]} {
      putserv "privmsg $chan :\002webby\002: Cookie \"$::durbyCookie\"..."
    } else {
...
proc durby {nick uhost handle chan site} {
   if {![channel get $chan durby]} { return }
   global durbyCookie ; set binary 1
   if {![info exists ::durbyCookie]} {
     set ::durbyCookie "" ; set cookies ""
   } else {
     set cookies $::durbyCookie
   }
   ...
   if {[regexp -nocase -- {--regexp (.*?)--} $site - reggy]} {
     if {[catch {set varnum [lindex [regexp -about -- $reggy] 0]} error]} {
       ...
       return 0
     } elseif {$varnum > $::durbyMaxRegexp} {
       putserv "privmsg $chan :\002regexp\002 too many captures ($varnum), reducing to ($::durbyMaxRegexp)"
       set varnum $::durbyMaxRegexp
     }
     set w5 0
   ...
   foreach {name value} $state(meta) {
     ...
     if {[string equal -nocase $name "Set-Cookie"]} {
       ...
       lappend durbyCookies [lindex [split $value {;}] 0]                                                                                                                                                             
     }                                                                                                                                                                                                             
   }
   if {[info exists durbyCookies] && [llength $durbyCookies]} {
     set cookies "[join $durbyCookies {;}]"
   } else {
     set cookies ""
   }
   ...
   if {[string equal $char $state(charset)] && [string equal $char $char2] && ![string equal -nocase "none given" $char]} {
     ...
     if {![string equal -nocase $char2 $char3] && ![string equal -nocase "none given" $char2] && $::durbyFixDetection > 0} {
       if {$::durbyFixDetection > 0} {
         switch $::durbyFixDetection {
           1 { if {![info exists w8]} {
                 if {$::durbyShowMisdetection > 0 } { set flaw "\002webby\002: conflict! html meta tagging reports: $char2 .. using charset detected from http-package: $char3 to avoid conflict."} { set flaw "" }
		     set html [durbyConflict $html $char3 $char2 2 [info exists w9]]
                 set char [string trim $char3 {;}]
               } else {
                 if {$::durbyShowMisdetection > 0 } { set flaw "\002webby\002: conflict! http-package reports: $char3 .. using charset detected from html meta tagging: $char2 to avoid conflict." } { set flaw "" }
		     set html [durbyConflict $html $char3 $char2 1 [info exists w9]]
   ...
   switch $::durbyDynEnc {
    ...
    2 { set html [incithencode $html [string map -nocase {"UTF-" "utf-" "iso-" "iso" "windows-" "cp" "shift_jis" "shiftjis"} $::durbyEnc]] }
   }
   ...
   foreach {name value} $state(meta) {
     ...
     if {[string match -nocase "last-modified" $name]} { set end " \)\( [durbyTime $value]" }
   ...
   if {[info exists w11]} {
	...
	if {[file exists $n]} {
      ...
      puts $f $fi
      close $f
      if {$::durbyFileReact < 1} { return }
   }
	
   # SPAM
   if {$::durbyShowMisdetection > 0} {
     if {[string length $flaw]} { putserv "privmsg $chan :$flaw" }
     if {$::durbyShowMisdetection > 1} {
       putserv "privmsg $chan :\002webby\002: Detected \002$char\002 \:\: Http-Package: $cset -> $char3 .. Meta-Charset: $mset -> $char2 \:\:"
     }
   }
   if {($::durbyRegShow > 0) || ![info exists w5]} {
     if {![string equal -nocase "$proto//$site" $url]} { set full "$url" } { set full "[durbytiny $fullquery $::durbyShortType]" }
     putserv "privmsg $chan : [durbydescdecode $title $char]"
     if {($::durbyheader > 0 && [llength $s]) || [info exists w1]} {
       foreach line [line_wrap [join [lsort -decreasing $s] "; "]] {
         putserv "privmsg $chan :$line"
       }
     }
     if {($::durbyXheader > 0 && [llength $sx]) || [info exists w2]} {
       foreach line [line_wrap [join [lsort -decreasing $sx] "; "]] {
         putserv "privmsg $chan :$line"
       }
     }
     if {($::durbydoc > 0 && [string length $hv]) || [info exists w3]} {
       foreach line [split [string trim $hv] \n] { putserv "privmsg $chan :$line" }
     }
     if {![info exists w3]} {
	regsub -all {(?:\n|\t|\v|\r|\x01)} $desc " " desc
       foreach line [line_wrap [durbydescdecode [durbydescdecode $desc $char] $char]] {
         putserv "privmsg $chan :$line"
       }
     }
   } else {
         ...
         for {set n 1} {$n<10} {incr n} {
           if {[info exists m($n)]} {
             ...
             if {([string length $m($n)] < $::regexp_limit) || [info exists w6]} {
               ...
               foreach line [line_wrap [durbydescdecode $m($n) $char]] {
		     regsub -all {(?:\n|\t|\v|\r|\x01)} $line " " line
...
proc durbyRangeString {length {chars "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"}} {
...
proc durbyConflict {html in out type gz} {
...
proc durbydescdecode {text char} {
...
proc durbytiny {url type} {
   set ua "Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.9.0.5) Gecko/2008120122 Firefox/3.0.5"
   set http [::http::config -useragent $ua -urlencoding "utf-8"]
   switch -- $type {
     4 { set type [rand 4] }
     5 { if {![info exists ::durbyCount]} {
           set ::durbyCount 0
           set type 0
         } else {
           set type [expr {[incr ::durbyCount] % 4}]
         }
       }
   } 
   switch -- $type {
...
proc durbyTime {stamp} {
	if {[catch { set ago [duration [expr {[clock seconds] - [clock scan "$stamp"] - $::durbyTimeAdjust}]] }]} {
		set ago [duration [expr {[clock seconds] - [clock scan "[join [lrange [split $stamp] 0 end-1]]"] - $::durbyTimeAdjust}]]
	}
...
# IS THE BOT PATCHED?!
# thanks thommey :P
proc durbyBotIsPatched { } { catch {botonchan #\uC0A0} e ; if {[string equal [string length $e] [string bytelength $e]]} { return 0 } { return 1 } }

putlog "Durby 2.0 :: Http: [package present http];BotIsPatched: [durbyBotIsPatched];Tls: [if {[catch {package present tls} e]} { set a "NO" } { set a [package present tls] }];Zlib: [if {[catch {package present zlib} e]} { set a "NO" } { set a [package present zlib] }];Trf: [if {[catch {package present trf} e]} { set a "NO" } { set a [package present trf] }];Gzip: [info exists durbyg];EncodingSystem: [encoding system];Tcl: $tcl_version;Eggdrop: [lindex $version 0];Suzi: [if {[info exists ::sp_version]} { set a "YES $::sp_version" } { set a "NO" }]"
