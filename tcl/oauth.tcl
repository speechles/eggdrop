...
# Get the inside look at every oauth transaction via putlog.
# enable debugging here 0=off/1=on
set oauthdebug 0
...
proc proc:twitter:oauth {url method chan query} {
	...
	if {$found != -1} {
		...
		if {$::oauthdebug > 0} { putlog "oauth:query_api $url $c_tok $c_sec $method $o_tok $o_sec $query" }
		set url [join [lrange [split $url :] 1 end]]
		if {[info exists twitter(tls)]} {
			if {$twitter(tls) > 0} {
				set proto "https:"
			} else { set proto "http:" }
		} else { set proto "http:" }
		if {[catch {set reply [oauth:query_api "$proto$url" $c_tok $c_sec $method $o_tok $o_sec $query]} error]} { error $error }
		return $reply
	}
}
...
proc oauth:query_call {url consumer_key consumer_secret method params {sign_params {}} {token {}} {token_secret {}}} {
	...
	if {$::oauthdebug > 0} { putlog "oauth_raw $oauth_raw" }
...
proc oauth:query {url method oauth_header {query {}}} {
	...
	if {[string match -nocase "::http::*" $error]} {
		...
		if {![string equal -nocase "utf-8" [encoding system]]} { set data [encoding convertto "utf-8" $data] }
		http::cleanup $token
		if {$ncode != 200} {
			set erta [list] ; # if {[string match *\\n* $data]} { set data "[lindex [split [string map [list \\n \n] $data] "\n"] 0]\}" }
			set junk [split [string map -nocase [list \" "" "\[" "" "\]" "" "\{" "" "\}" ""] [join $data]] ,]
			foreach name $junk {
				lappend erta [string map [list ":" ": " "\\" ""] [string totitle [string trim $name]]]
			}
			if {[llength $erta]} { 
...
proc oauth:signature {url consumer_secret method params {token_secret {}}} {
	...
	if {$::oauthdebug > 0} { putlog "base-string $base_string" }
