...
# +-+
variable webbyShowUrls 0
# +-+
# Use verbose mode by default?
# if enabled webby will always show type information and description of url
...
# --- [ 0 off / 1 on ]
# +-+
variable webbyVerbose 0
# +-+
#
# grab urls from channel messages?
# this will set webby to watch for urls in channel and
# post url information when one is found.
# +-+
variable webbyUrlWatch 1
# +-+
#
# Multiple urls in one line of text? 0 = first is all shown, 1 = more than 1 can be used per line
# --- [ 0 off / 1 on ]
# +-+
variable webbyCollectTitles 1
# +-+
# <--- end of config; script begins

package require http
if {![catch {package require tls}]} { set webbyg 1; ::http::register https 443 ::tls::socket }
...
proc weburlwatch {nick host user chan text} {
	...
	foreach word "[split $text]" {
		if {[string match -nocase "!webby*" $word] || [string match -nocase "!webby*" $word]} {
			return 0
...
proc webby {nick uhost handle chan site} {
   ...
   if {($::webbyRegShow > 0) || ![info exists w5]} {
     ...
     putserv "privmsg $chan : [webbydescdecode $title $char]"
...
# IS THE BOT PATCHED?!
# thanks thommey :P
proc webbyBotIsPatched { } { catch {botonchan #\uC0A0} e ; if {[string equal [string length $e] [string bytelength $e]]} { return 0 } { return 1 } }

putlog "webby 2.0 :: Http: [package present http];BotIsPatched: [webbyBotIsPatched];Tls: [if {[catch {package present tls} e]} { set a "NO" } { set a [package present tls] }];Zlib: [if {[catch {package present zlib} e]} { set a "NO" } { set a [package present zlib] }];Trf: [if {[catch {package present trf} e]} { set a "NO" } { set a [package present trf] }];Gzip: [info exists webbyg];EncodingSystem: [encoding system];Tcl: $tcl_version;Eggdrop: [lindex $version 0];Suzi: [if {[info exists ::sp_version]} { set a "YES $::sp_version" } { set a "NO" }]"
