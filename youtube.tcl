# YouTube (Eggdrop/Tcl), Version 1.5
#
# (-) speechles(Efnet - #roms-isos), 27. Jul. 2013
#
# Example:
...
proc YouTube {nick host hand chan text} {
		...
		if {[catch {package require http 2.5}]} {
			...
			if {[regexp -nocase {(^|[ ]{1})(https{0,1}:\/\/(www\.){0,1}|www\.)(youtu\.be\/|youtube\.com\/watch[^ ]{1,}v=)([A-Za-z0-9_-]{11})} $text - - - - - y_vid]} {
				if {[catch {set y_con [::http::geturl $y_api$y_vid -headers [list {GData-Version} {2}] -timeout 5000]}]} {
					...
					if {[youtubeBotIsPatched]} {
						if {[catch {package present http 2.7}]} {
							set y_data [encoding convertto utf-8 [::http::data $y_con]]
						} else {
							set y_data [encoding convertto utf-8 [::http::data $y_con]]
						}
					} else {
						if {[info exists ::sp_version]} {
							set y_data [encoding convertfrom utf-8 [http::data $y_con]]
						} elseif {[catch {package present http 2.7}]} {
							set y_data [encoding convertto utf-8 [encoding convertfrom utf-8 [::http::data $y_con]]] 
						} else {
	...
	if {[info exists y_data]} {
		...
		if {[string length $de] > $::youtubeCut && $::youtubeCut > 0} { set de "[string range $de 0 [string first " " $de $::youtubeCut]]... " }
		...
		#youtubeMsg privmsg $chan "\002\00301,00 #You\003\00300,04Tube \003\002 \002\00304$nick\003\002: \002\00302[youtubeMap $n]\003\002 \00302( by\003 \002\00302$a\003\002 | \002\00302$rd\003\002 | \002\00302$r\003\002 \00302rating\003 | \002\00302[youtubeCommify $c]\003\002 \00302comments\003 | \002\00302[youtubeCommify $v]\003\002 \00302views\003 | \002\00302$ago\003\002\00302)\003 - \00302[string trimleft [youtubeMap $de] " |"]\003"
...
proc youtubeStrip {str} {
    return [regsub -all -- {\017|\035|\002|\037|\026|\003(\d{1,2})?(,\d{1,2})?} $str ""]
}

# IS THE BOT PATCHED?!
# thanks thommey :P
proc youtubeBotIsPatched { } { catch {botonchan #\uC0A0} e ; if {[string equal [string length $e] [string bytelength $e]]} { return 0 } { return 1 } }
...
proc youtubeMsg {type dest data} {
   ...
   foreach line [youtubeWordwrap $data $len] {
        ...
        set fix ""
        # add bold
        if {[expr {[youtubeCountChars \002 $lastline] & 1 }]} {
          append fix "\002"
        }
        # add underline
        if {[expr {[youtubeCountChars \037 $lastline] & 1 }]} {
          append fix "\037"
        }
        # add color
        if {[expr {[youtubeCountChars \003 $lastline] & 1 }]} {
        ...
        if {[string length [string map {" " ""} [youtubeStrip $line]]]} {
          putserv "$type $dest :$fix$line"
        }
        set lastline "$fix$line"
   }
}

# count characters appearing in text
# regexp is unreliable for utf-8, go figure...
proc youtubeCountChars {char text} {
   set counted 0
   foreach letter [split $text ""] {
	if {[string equal $char $letter]} {
	   incr counted
      }
   }
   return $counted
...
putlog "YouTube 1.5 :: Http: [package present http];BotIsPatched: [youtubeBotIsPatched];EncodingSystem: [encoding system];Tcl: $tcl_version;Eggdrop: [lindex $version 0];Suzi: [if {[info exists ::sp_version]} { set a "YES $::sp_version" } { set a "NO" }]"
